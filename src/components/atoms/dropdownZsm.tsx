...
export default function Example({ dropDownList, selectedVal }) {
	const people = dropDownList;

	const [selected, setSelected] = useState('Select Cluster');

	useEffect(() => {
		dropDownList?.length !== 0 && setSelected(dropDownList[0]?.name);
	}, [dropDownList]);

	const onChangeValue = async (val) => {
		await setSelected(val.name);
		await selectedVal(val);
		// localStorage.setItem('selectedDistrict',val)
	};
	return (
